...
class PortUtilTest {
  ...
  private fun startListeningProcess(port: Int): Process {
    ...
    pb.redirectOutput(ProcessBuilder.Redirect.to(File.createTempFile("listening-app-$port", ".log")))
    return pb.start()
  }

  private fun startConnectingProcess(port: Int, idx: Int): Process {
    val classpath = System.getProperty("java.class.path")
    val java = getJavaBin()
    val pb = ProcessBuilder(
      listOf(
    ...
    pb.redirectErrorStream(true)
    pb.redirectOutput(ProcessBuilder.Redirect.to(File.createTempFile("connecting-app-$port-$idx", ".log")))
    return pb.start()
  }
  ...
  fun getProcessesUsingPort_returnsMultiplePids_whenTwoClientsConnected() {
    ...
    ServerSocket(port, 0, InetAddress.getLoopbackAddress()).use {
      val c1 = startConnectingProcess(port, 1)
      val c2 = startConnectingProcess(port, 2)
  ...
  fun killProcessesUsingPort_killsAllChildProcesses_whenListenerAndTwoClientsUsePort() {
    ...
    val client1 = startConnectingProcess(port, 1)
    val client2 = startConnectingProcess(port, 2)
